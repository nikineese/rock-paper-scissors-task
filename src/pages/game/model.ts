...
import { elementModel, GameType, StatusType } from "../../entities";
...
export const gameStartAgain = merge([
  statusReset,
  socketApi.opponentDisconnected,
]);
...
export const $opponentStatus = restore(
  statusesChanged,
  StatusType.IN_GAME
).reset(gameStartAgain);
export const $players = restore(playersChanged, []);

export const calculateWinnerFx = createEffect((results: GameType[]) => {
  const combinations: { [key: string]: elementModel.ChooseType } = {
    paper: elementModel.ChooseType.ROCK,
    scissors: elementModel.ChooseType.PAPER,
    rock: elementModel.ChooseType.SCISSORS,
  };
